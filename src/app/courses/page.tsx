      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="p-8 sm:p-10 rounded-3xl bg-gradient-to-r from-amber-500 via-amber-600 to-amber-700 text-white shadow-xl space-y-4 border border-amber-400">
          ...
          <p className="text-sm font-light text-amber-50 max-w-3xl leading-relaxed">
            Our self-paced asynchronous video course library is currently under construction and undergoing content verification. In the meantime, you can book live 1-on-1 peer mentoring lessons directly with competition winners!
          </p>
          <div className="pt-2">
            <Link
              href="/mentors"
            ...
            >
              Book 1-on-1 Mentor Session ($75/hr) <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        </div>
          ...
          {courses.map((c) => (
            <div key={c.id} className="bg-white rounded-3xl border border-slate-200 overflow-hidden flex flex-col justify-between shadow-sm">
              ...
              <div className="p-6 pt-0">
                <button
                  onClick={() => alert("Asynchronous courses are currently under construction.")}
                  className="w-full py-3 rounded-full bg-slate-900 text-white text-xs font-normal opacity-90 hover:opacity-100"
                >
